...
from submodules.LigandMPNN.model_utils import ProteinMPNN
from submodules.LigandMPNN.data_utils import restype_str_to_int, restype_1to3, restype_int_to_str
...
class Potential_from_bb:
    ...
    def __init__(self, binderlen=-1, threshold=3, seq_model_type='ligand_mpnn'):
        ...
        seq_checkpoint = torch.load(checkpoint_path, map_location='cpu')
        ...
        self.seq_model.to(torch.cuda.current_device())
        self.seq_model.eval()
        print('Load LigandMPNN model')

        # Initialize BB2Martini
        self.recover_sc=BB2Martini(chains= ['_p1'] if binderlen<0 else ['_p1','_p2'])

        # Initialize dMaSIF
        checkpoint_path='models/martini_prot_from_bb_nnthr'
        surf_checkpoint=torch.load(checkpoint_path, map_location='cpu')
        self.surf_model=dMaSIF(surf_checkpoint['net_args'])
        self.surf_model.load_state_dict(surf_checkpoint["model_state_dict"])
        self.surf_model.to(torch.cuda.current_device())
    ...
    def run_LigandMPNN(self, xyz):
        ...
        feature_dict["S"] = torch.zeros((1, L)).to(device)# encoded sequence
    
        feature_dict["X"] = xyz[None,:,:4,:] # B*L*4*3 (bb atoms)  ? normalize

        feature_dict["mask"] = torch.ones([1, L]).to(device)
        feature_dict["chain_mask"] = torch.ones([1, L]).to(device)
        feature_dict["temperature"] = 0.1
        feature_dict["bias"] = torch.zeros((1,L,21)).to(device)
        feature_dict["randn"]=torch.randn((1,L)).to(device)
        feature_dict["symmetry_residues"] = [[]]
        feature_dict["symmetry_weights"]=[[]]
        feature_dict["Y"] = torch.zeros([1, L, 16, 3]).to(device)
        feature_dict["Y_t"] = torch.zeros([1, L, 16]).to(device)
        feature_dict["Y_m"] = torch.zeros([1, L, 16]).to(device)

        feature_dict["R_idx"] = torch.arange(L)[None,:].to(device) # B*L resnums

        if self.binderlen>0:
            feature_dict["chain_labels"] = torch.cat((torch.zeros((1,self.binderlen)),
                                              torch.ones((1,L-self.binderlen))),1).to(device)  # B*L Chain indices
        else:
            feature_dict["chain_labels"]=torch.zeros((1,L)).to(device)
    ...
    def gen_labels(self, d):
        ...
        if self.binderlen<0:
            ...
            P1['labels'] = (xyz_dists.sum(1) > 1.0).float().view(-1).detach()
            P2['labels'] = (xyz_dists.sum(0) > 1.0).float().view(-1).detach()
    ...
    def calc_loss(self, P1, P2, lf=F.binary_cross_entropy_with_logits):
        ...
        if self.binderlen<0:
            ...
            if P1["edge_labels"].shape[0]>0:
                ...
                loss+=lf(pos_preds, pos_labels, reduction='mean')

            neg_preds1=P1['preds'][P1['labels']==0]
            if neg_preds1.shape[0]>0:
                neg_labels1=torch.ones_like(neg_preds1)
                loss+=lf(neg_preds1, neg_labels1, reduction='mean')

            neg_preds2=P2['preds'][P2['labels']==0]
            if neg_preds2.shape[0]>0:
                neg_labels2=torch.ones_like(neg_preds2)
                loss+=lf(neg_preds2, neg_labels2, reduction='mean')

        return loss

    def __call__(self, xyz):

        seq=get_pot.get_aa_probs(xyz)

        d=get_pot.bb2martini(xyz, seq)

        d=get_pot.dmasif(d)

        P1, P2=get_pot.gen_labels(d)

        loss=get_pot.calc_loss(P1, P2)
